		...
		for (Map.Entry<String, ClassMetadata> entry : map.entrySet()) {
			...
			for (String name : names) {
				...
				if (type instanceof ManyToOneType) {
					...
					list.add(new Tuple<Class<?>, String>(cm.getMappedClass(), name));
						...
						while (true) {
							for (Field f : superClass.getDeclaredFields()) {
								if (f.getAnnotation(ManyToOne.class) != null
									...
									list.add(new Tuple<Class<?>, Tuple<String, String>>(cm.getMappedClass(),
											new Tuple<String, String>(name, f.getName())));
								}
							}
