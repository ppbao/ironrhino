...
public class DES {
	...
	public static String encryptWithSalt(String str, String salt) {
		...
		try {
			return new String(Base64.encodeBase64(des.encrypt(str.getBytes(StandardCharsets.UTF_8))),
					StandardCharsets.UTF_8);
		} catch (Exception ex) {
			logger.error("encrypt exception!", ex);
			return "";
	...
	public static String decryptWithSalt(String str, String salt) {
		DES des = new DES(defaultKey + salt);
		try {
			return new String(des.decrypt(Base64.decodeBase64(str.getBytes(StandardCharsets.UTF_8))),
					StandardCharsets.UTF_8);
		} catch (Exception ex) {
			logger.error("decrypt exception!", ex);
			return "";
